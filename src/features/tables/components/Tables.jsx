...
export const Tables = () => {
    const { tables, loading, getTables, deleteTable } = useAdminStore();
    ...
    return (
        <div className="p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                {tables.map((table) => (
                    <div key={table._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col items-center hover:shadow-md transition-all">
                        <div className="w-20 h-20 bg-main-blue/10 rounded-full flex items-center justify-center mb-4 mt-2 border-2 border-main-blue/5">
                            <span className="text-3xl font-black text-main-blue">
                                {table.tableNumber < 10 ? `0${table.tableNumber}` : table.tableNumber}
                            </span>
                        </div>
                        
                        <div className="text-center mb-4">
                            <h3 className="text-lg font-bold text-gray-800 leading-tight">Mesa #{table.tableNumber}</h3>
                            <p className="text-[10px] text-main-blue font-bold uppercase tracking-wider">
                                {table.sucursalName || "Sucursal Central"}
                            </p>
                        </div>
            ...
            {isModalOpen && <TableModal onClose={() => setIsModalOpen(false)} />}
        </div>
    );
};